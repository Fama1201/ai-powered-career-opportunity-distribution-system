// hr-notifications.js
// HR Notifications page functionality
...
async function initNotifications() {
    // Don't check authentication here - let the page load normally
    // If there's an auth issue, it will be handled gracefully in loadNotifications
    loadSavedTheme();
    loadSavedLanguage();
...
async function loadNotifications() {
    ...
    try {
        // Use StudentAPI for notifications (same endpoint works for HR)
        ...
        // Never redirect to login - just show error message and empty state
        // User can still use the page even if notifications fail to load
        if (typeof UI !== 'undefined' && UI.toast) {
            // Only show error toast for non-auth errors to avoid confusion
            if (!error.message || (!error.message.includes('Unauthorized') && !error.message.includes('Not authenticated'))) {
                UI.toast(error.message || 'Failed to load notifications. Please try again.', 'error');
            }
        }
        if (emptyState) {
...
async function handleMarkAsRead(notificationId, cardElement) {
    try {
        ...
        if (typeof UI !== 'undefined' && UI.toast) {
            UI.toast(error.message || 'Failed to mark notification as read. Please try again.', 'error');
        }
    }
}
